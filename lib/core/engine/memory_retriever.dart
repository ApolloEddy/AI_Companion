...
class LayeredMemoryRetriever {
  ...
  Future<RetrievalResult> retrieve({
    ...
    result.episodicMemory = await _retrieveEpisodicMemory(
      query,
      perception,
    );
  ...
  Future<List<EpisodicMemory>> _retrieveEpisodicMemory(
    ...
    // 从 MemoryManager 获取存储的记忆
    final allMemories = _memoryManager.getAllMemories();
    
    // 基于关键词匹配计算相关性
    for (int i = 0; i < allMemories.length; i++) {
      final mem = allMemories[i];
      final relevance = _calculateRelevance(mem, query);
      
      if (relevance > 0.3) {
        memories.add(EpisodicMemory(
          content: mem,
          timestamp: DateTime.now().subtract(Duration(hours: allMemories.length - i)),
  ...
  /// 计算相关性分数
  double _calculateRelevance(String memory, String query) {
    // 简单的关键词匹配
    ...
    for (final word in queryWords) {
      if (word.length > 1 && memLower.contains(word)) {
        matches++;
      }
    }
  ...
  RetrievalResult _compressResult(RetrievalResult result, int maxTokens) {
    ...
    while (episodicTokens > episodicBudget && episodicMemory.length > 1) {
      ...
      episodicTokens = episodicMemory.fold(0, (sum, e) => sum + (e.content.length / 1.5).ceil());
    }
